...
import { tool } from 'ai'
import { z } from 'zod'
import { EmptyDataError } from '@traderalice/opentypebb'
...
function buildContext(
  ...
  return {
    getHistoricalData: async (symbol, interval) => {
      ...
      const fetchRaw = async (sym: string, intvl: string, overrideProvider?: string) => {
        const params: Record<string, unknown> = { symbol: sym, start_date, interval: intvl }
        if (overrideProvider) params.provider = overrideProvider

        switch (asset) {
          case 'equity':
            return await equityClient.getHistorical(params)
          case 'crypto':
            return await cryptoClient.getHistorical(params)
          case 'currency':
            return await currencyClient.getHistorical(params)
          case 'commodity':
            return await commodityClient.getSpotPrices(params)
        }
      }
      ...
      try {
        raw = await fetchRaw(symbol, interval)
      } catch (err) {
        // If primary provider (yfinance) has no data, try FMP as fallback
        if (err instanceof EmptyDataError) {
          try {
            raw = await fetchRaw(symbol, interval, 'fmp')
          } catch {
            throw new Error(`No historical data returned for '${symbol}' from any available provider`)
          }
        } else {
          throw err
        }
