 ...
 * OpenBB SDK expects:                { "federal_reserve_api_key": "abc123" }
 *                                       (Provider constructor prepends "federal_reserve_")
 *
 * OpenBB extractors (getFredApiKey, etc.) read unprefixed names
 * (fred_api_key, eia_api_key). The executor wrapper restores these
 * from the provider-prefixed names after filterCredentials runs.
 */

const keyMapping: Record<string, string> = {
  fred: 'federal_reserve_api_key',
  fmp: 'fmp_api_key',
  eia: 'eia_eia_api_key',
  bls: 'bls_api_key',
